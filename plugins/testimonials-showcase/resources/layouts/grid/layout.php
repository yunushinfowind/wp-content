	...
	public function build_layout( $query = array() , $options = array(), $preview = false ) {
		...
		//Rich Snippets Variables 
			...
			if($snippet_on) {
				...
				$item_logo = cmshowcase_get_option( 'schema_logo', 'ttshowcase_rich_snippets' );
				$item_telephone = cmshowcase_get_option( 'schema_telephone', 'ttshowcase_rich_snippets' );
				$item_address = cmshowcase_get_option( 'schema_business_address', 'ttshowcase_rich_snippets' );
    ...
    <span itemprop="name">' . $sub_title . '</span></span></div>';
				...
				*/
				...
				$template_replace = array($quote, $read_more, $review_title, $title, $date, $sub_title, $stars, $designation, $taxcategory); 
					...
					if($snippet_on) {
						...
						$html .= '<div itemprop="itemReviewed" itemscope itemtype="http://schema.org/LocalBusiness">
							<meta itemprop="name" content="'.$itemreviewed.'">
							<meta itemprop="image" content="'.$item_logo.'"  />
							<meta itemprop="address" content="'.$item_address.'"  />
							<meta itemprop="telephone" content="'.$item_telephone.'"  />
						</div>';
					}
		...
		if($snippet_on && $review_count >0 ) {
			...
			$html .= '<div itemscope itemtype="http://schema.org/AggregateRating">
					   ...
					   <meta itemprop="reviewCount" content="'.$count_posts = wp_count_posts( 'ttshowcase' )->publish.'">
					  </div>';
