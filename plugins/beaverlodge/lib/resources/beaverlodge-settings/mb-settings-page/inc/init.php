...
{
	global $mb_settings_meta_boxes;
	$mb_settings_meta_boxes = array();
	foreach ( $meta_boxes as $k => $meta_box )
	{
		if ( isset( $meta_box['settings_pages'] ) )
		{
			unset( $meta_box['post_types'], $meta_box['pages'] );
			$mb_settings_meta_boxes[$k] = $meta_box;
...
/**
 * Initialize meta boxes for taxonomy.
 * 'rwmb_meta_boxes' runs at priority 10, we use priority 20 to make sure global variable $mb_settings_meta_boxes is set.
 * @see mb_settings_page_filter()
 */
add_action( 'admin_init', 'mb_settings_page_init', 20 );

/**
 * Register meta boxes for taxonomy, each meta box is a section
 * @return void
 */
function mb_settings_page_init()
{
	...
	// Prevent errors showing if invalid value is returned from the filter above
	if ( empty( $mb_settings_meta_boxes ) || ! is_array( $mb_settings_meta_boxes ) )
		return;

	foreach ( $mb_settings_meta_boxes as $meta_box )
	{
