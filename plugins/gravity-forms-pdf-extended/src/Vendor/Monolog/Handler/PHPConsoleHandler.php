...
use GFPDF\Vendor\PhpConsole\Connector;
use GFPDF\Vendor\PhpConsole\Handler;
use GFPDF\Vendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \GFPDF\Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \GFPDF\Vendor\PhpConsole\Connector $connector = null, $level = \GFPDF\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('GFPDF\\Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\GFPDF\Vendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \GFPDF\Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \GFPDF\Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\GFPDF\Vendor\PhpConsole\Helper::isRegistered()) {
            \GFPDF\Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \GFPDF\Vendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \GFPDF\Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \GFPDF\Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \GFPDF\Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
